            ...
            return (
              ...
              >
                {formatter && item?.value !== undefined && item.name ? (
                  formatter(item.value as React.ReactNode, item.name as string, item, index, payload as Array<Record<string, unknown>>)
                ) : (
                  <>
                    {!hideIndicator && (
                      <div
...
function getPayloadConfigFromPayload(
  ...
  const configLabelKey = Object.entries(config).find(
    ([, config]) => config.label === payload.name
  )?.[0];
