    ...
    <div className="flex min-h-screen bg-[#F7FAF8]">
          ...
          {!collapsed && (
            <Link href="/dashboard" className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="flex flex-col min-w-0">
                ...
                <span className="text-[9px] font-semibold text-[#9CA3AF] uppercase tracking-wider leading-tight truncate">Administrasi Sistem</span>
              </div>
            </Link>
          )}
        ...
        {!collapsed && (
          <div className="mx-3 mb-3 overflow-hidden rounded-xl bg-[var(--hafta-ylw-50)]">
            <div className="relative h-20 w-full overflow-hidden">
              <Image
                src="/images/models/model-patient-1.png"
                alt=""
                fill
      ...
      <div
        className={cn(
          ...
          // Mobile tidak pakai padding-left (sidebar overlay)
          'lg:pl-[240px]',
          // Desktop: sesuaikan berdasarkan collapsed state
          collapsed && 'lg:pl-[64px]',
        )}
      >
        {/* Top header */}
        <header className="sticky top-0 z-30 flex h-[64px] items-center gap-4 border-b border-[#E5E7EB] bg-white/90 backdrop-blur-md px-4 lg:px-6">
          {/* Mobile menu button */}
          <Button
            variant="ghost"
            size="icon"
          ...
          {/* Search */}
          <div className="hidden md:flex flex-1 max-w-lg items-center gap-2 rounded-xl border border-[#E5E7EB] bg-[#F7FAF8] px-4 py-2 transition-colors focus-within:border-[var(--hafta-ylw-200)] focus-within:bg-white focus-within:shadow-sm">
            <Search className="h-4 w-4 text-[#9CA3AF] shrink-0" />
            <input
              type="text"
              placeholder="Cari pasien, nomor rekam medis, atau nama..."
            ...
            <DropdownMenu>
              <DropdownMenuTrigger className="flex items-center gap-2.5 rounded-xl p-1.5 transition-colors hover:bg-[#F1F5F9] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--hafta-ylw-200)]">
                ...
                <div className="hidden min-w-0 flex-col text-left sm:flex">
                  <span className="truncate text-[13px] font-semibold text-[#111827]">{userName || 'Administrator'}</span>
                  <span className="truncate text-[11px] text-[#6B7280]">@{userEmail || 'admin'}</span>
                </div>
                <ChevronDown className="hidden h-4 w-4 shrink-0 text-[#9CA3AF] sm:block" />
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-56">
                <div className="px-3 py-2">
                  <p className="text-sm font-semibold text-[#111827]">{userName || 'Administrator'}</p>
                  <p className="text-xs text-[#6B7280]">@{userEmail || 'admin'}</p>
                </div>
                <DropdownMenuSeparator />
...
// Unused var helper — for dead-code elimination
void (SIDEBAR_FULL + SIDEBAR_COLLAPSED);
